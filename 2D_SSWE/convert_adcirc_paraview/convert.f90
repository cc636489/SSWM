module convert
   ...
   subroutine cpp_transform(new_lat, new_lon, x_dis, y_dis)
       implicit none
       real(kind=8), intent(in)   :: new_lat, new_lon
       real(kind=8), intent(out)  :: x_dis, y_dis
       real(kind=8)               :: r_earth, center_lon, center_lat, deg_2_rad 
       r_earth = 6378206.4
       center_lat = 18.183754 
       center_lon = -97.796768
       deg_2_rad = 3.1415926 / 180.0
       x_dis = r_earth * (new_lon - center_lon) * deg_2_rad * cos(center_lat * deg_2_rad)
   ...
   subroutine extract_global_data_from_fort14(fort14_filename, the_data)
      ...
      do i1 = 1, the_data%NumNd, 1
         read (unit=14, fmt=*) the_data%NdIDs(i1), &
            tmpa, tmpb, the_data%bathymetry(i1)
         call cpp_transform(tmpb, tmpa, the_data%NdCoords((i1 - 1)*dim1 + 1), &
...
program main
  ...
  call extract_global_data_from_fort63("minpr.63", global_data)
 ! call extract_global_data_from_fort64("fort.74", global_data)
 ...
 !                                  "wvel", "wvel.pvd")
 ! call extract_global_data_from_fort63("fort.73", global_data)
 ! call extract_global_data_from_fort64("fort.74", global_data)
 ! call write_node_series_1D_field_to_vtu_pvd(global_data, global_data%elevation, global_data%timestamp, &
 !                                  "minpre", "minpre.pvd")
 ! call write_node_series_2D_field_to_vtu_pvd(global_data, global_data%uvelocity, global_data%timestamp, &
