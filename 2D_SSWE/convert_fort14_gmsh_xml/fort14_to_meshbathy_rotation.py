...
inputfile = "Gulf_wind.fort.14"
outputfile = "Gulf_wind.msh"
bathymetryfile = "Gulf_wind_bathyemtry.nc"
DEBUG = True
Cartesian = False
Rotation = False
theta = np.pi/4
...
with open(inputfile) as f_input:
    ...
    for i, thisline in enumerate(f_input):
        ...
        if (i > 1) and (i <= nnode+1):
            ...
            if Cartesian:
                if Rotation:
                    x0 = float(words[1])
                    y0 = float(words[2])
                    x_distance = np.cos(theta)*x0 - np.sin(theta)*y0
                    y_distance = np.cos(theta)*y0 + np.sin(theta)*x0
                else:
