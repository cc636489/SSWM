...
name = "test4"
input_dir = "/Users/chenchen/gloria/4-NS-results-and-tests/regression_test_stochastic/test4_stochastic/"
output_dir = "/Users/chenchen/gloria/test_fenics/stochastic/test4_stochastic_inlet/"
mesh_dir = "/Users/chenchen/gloria/7-SSWM-github/input/"
mesh_file = "inlet_adh_sswm_finer.xml"
st_order = 3
u_file = "u_used_for_read_back_"+name.lower()+"_stochastic_0.2_0.75_0.13les_"
eta_file = "eta_used_for_read_back_"+name.lower()+"_stochastic_0.2_0.75_0.13les_"
...
#    import pdb;pdb.set_trace()
    ...
    for i, item in enumerate(test_nodes): 
        ...
        # save to file
        np.save(output_dir + "binrandom_surrogate_u_samples_order_"+str(item[0])+"_"+str(item[1])+"_"+str(time_step-1)+"_NSample_"+str(n_sample), u_random_output)
        np.save(output_dir + "binrandom_surrogate_v_samples_order_"+str(item[0])+"_"+str(item[1])+"_"+str(time_step-1)+"_NSample_"+str(n_sample), v_random_output)
        np.save(output_dir + "binrandom_surrogate_eta_samples_order_"+str(item[0])+"_"+str(item[1])+"_"+str(time_step-1)+"_NSample_"+str(n_sample), eta_random_output)
