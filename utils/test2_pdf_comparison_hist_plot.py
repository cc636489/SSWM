...
input_dir = "/Users/chenchen/gloria/test_fenics/stochastic/test2_stochastic_sloth/"
truth_dir = "/Users/chenchen/gloria/4-NS-results-and-tests/regression_test_stochastic/test2_bins/"
name = "eta"
...
for i,order in enumerate(order_list):
    ...
    truth_name = "test2_bin_random_"+name+"1_true_all_points_order_1.npy"
    ...
    sns.distplot(truth[:, :, 40, 0],label="truth", color=color[i+1]) # timestep=40, testnode number = 0.
    ensemble_mean = np.mean(ensembles)
    ensemble_var = np.sqrt(np.var(ensembles))
    truth_mean = np.mean(truth[:,:,40,0])
    truth_var = np.sqrt(np.var(truth[:,:,40,0]))
    print("test:============= "+name+" ========")
