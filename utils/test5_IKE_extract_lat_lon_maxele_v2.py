...
plot_eta_maxmean_var = np.zeros([len(test_nodes), 2])
plot_eta_ADCIRC_max = np.zeros([len(test_nodes)])

f = open("test5_maxmean_var_node_ADCIRC.txt", "w")
f.write("node_number, SSWM_maxmean, SSWM_with_its_var, SSWM_mean_plus_var, ADCIRC_maxele\n")
for i in range(len(test_nodes)):
    #import pdb; pdb.set_trace()
    plot_eta_ADCIRC_max[i] = np.max(tmp_obs[i][:])
    plot_eta_maxmean_var[i, 0] = np.max(plot_eta_mean[:, i])
    max_mean_index = np.argmax(plot_eta_mean[:, i])
    plot_eta_maxmean_var[i, 1] = plot_eta_var[max_mean_index, i]
    line = test_node_str_node_number[i] + " , " + str(plot_eta_maxmean_var[i, 0]) + " , " + str(plot_eta_maxmean_var[i, 1]) + " , " + str(plot_eta_maxmean_var[i, 0] + plot_eta_maxmean_var[i, 1]) + " , " + str(plot_eta_ADCIRC_max[i]) + "\n"
    print line
    f.write(line)
f.close()
