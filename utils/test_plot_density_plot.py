...
u_file = "u_used_for_read_back_gulf_winds_"+name.lower()+"_stochastic_"
eta_file = "eta_used_for_read_back_gulf_winds_"+name.lower()+"_stochastic_"
emin = -1.0
emax = 1.0
umin = -0.45
umax = 0.45
...
test_node_x = [346974.525421, 582699.363816, 642084.289864]
test_node_y = [3278821.479, 3280530.02913, 3277486.52114]
test_node_str =["galveston entrance", "right coast", "the most right coast"]
test_node_str_node_number = ["5171", "3516", "3702"]
        ...
        for j, field in enumerate([u_random_output, v_random_output, eta_random_output]):
            ...
            try:
                ...
                if j == 0:
                    ...
                    plt.title('Pdf of x-direction water velocity at time = ' + str(k * step_size /3600.0/24.0) + 'days;')
                    plt.savefig(output_dir + 'u_display_density_plot_testnode_' + str(item[0]) + '_' + str(item[1]) +'_timestep_'+str(k)+'.pdf')
                elif j == 1:
                    sns.distplot(field)
                    plt.xlim(vmin, vmax)
                    ...
                    plt.title('Pdf of y-direction water velocity at time = ' + str(k * step_size/3600.0/24.0) + 'days;')
                    plt.savefig(output_dir + 'v_display_density_plot_testnode_' + str(item[0]) + '_' + str(item[1]) +'_timestep_'+str(k)+'.pdf')
                else:
                    sns.distplot(field)
                    plt.xlim(emin, emax)
                    ...
                    plt.title('Pdf of surface elevation at time = ' + str(k * step_size/3600.0/24.0) + 'day;')
                    plt.savefig(output_dir + 'eta_display_density_plot_testnode_' + str(item[0]) + '_' + str(item[1]) +'_timestep_'+str(k)+'.pdf')
                plt.close()
            except:
