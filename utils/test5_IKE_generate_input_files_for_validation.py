...
input_file = "input_generalized_testcase_stochastic_Gulf_wind_IKE.py"
...
for q0 in np.linspace(0.8, 1.2, num = 50):
    #for q1 in np.linspace(0.9, 1.1, num = 10):
        ...
        output_file = input_file[:27] + "deterministic_IKE_" + str(round(q0, 2)) + ".py"
        subprocess.call(["cp " + input_dir + input_file + " " + output_dir1 + output_dir2 + output_file], shell = True)
        subprocess.call(["sed -i 's#output_dir = \"/workspace/Documentation/Research_Doc/SFEM_Doc/4-NS-results-and-tests/regression_test_stochastic/\"#output_dir = \"/workspace/Documentation/Research_Doc/SFEM_Doc/4-NS-results-and-tests/regression_test_stochastic/IKE_truth/\"#' " + output_dir1 + output_dir2 + output_file], shell = True)
        subprocess.call(["sed -i 's/output_str = \"gulf_winds_ike_stochastic_\"/output_str = \"gulf_winds_ike_deterministic_" + str(round(q0, 2)) + "_\"/' " + output_dir1 + output_dir2 + output_file], shell = True)
