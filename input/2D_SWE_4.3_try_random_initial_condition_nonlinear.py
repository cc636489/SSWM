...
prefix = "/workspace/Documentation/Research_Doc/SFEM_Doc/4-NS-results-and-tests/" \
         "test8_SUPG_SamgLily_crosswind/test4_supg_cross_try_randominit_wiles_wifric"
...
class InitialConditions(Expression):
    ...
    def eval(self, values, x):
        values[0] = - 1e-6 + (random.random()-0.1)/0.9 * 2 * 1e-6
        values[1] = - 1e-6 + (random.random()-0.1)/0.9 * 2 * 1e-6

    def value_shape(self):
        return (2,)


initial_condition_u = InitialConditions(degree=2)
...
if include_crosswind:
    ...
    u_bash_parallel = inner(u_bash, grad(u_mean[0])) * grad(u_mean[0]) / du_norm ** 2
    v_bash_parallel = inner(u_bash, grad(u_mean[1])) * grad(u_mean[1]) / dv_norm ** 2
...
# make a functional into a vector function
R = action(F_u_tent, ut)
# automatic differentiation
DR = derivative(R, ut)  # Gateaux derivative
# compute solution
problem = NonlinearVariationalProblem(R, ut, bcu, DR)
